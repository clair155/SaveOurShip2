...
{
	class CompShipScannerAI : CompShipScanner
	{
		...
		{
			base.CompTick();
			if (Find.TickManager.TicksGame % 250 != 0 || !this.parent.Map.IsSOS2Space() || !(this.powerComp?.PowerOn ?? true) || this.parent.Faction != Faction.OfPlayer)
				return;
