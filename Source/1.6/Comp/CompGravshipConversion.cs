...
{
	class CompGravshipConversion : ThingComp
	{
		...
		{
			if (!(parent.TryGetComp<CompFlickable>()?.SwitchIsOn ?? true) || !parent.Spawned || parent.Map.IsSpace() && parent.Map.GetComponent<ShipMapComp>().ShipMapState != ShipMapState.nominal || parent.Map.mapPawns.AllPawns.Where(p => !p.IsPrisoner && !p.InContainerEnclosed && p.HostileTo(Faction.OfPlayer)).Any())
			{
				return;
			}
			age++;
