...
namespace SaveOurShip2
{
	class CompArchoHullConversion : ThingComp
	{
		...
		{
			if (!OptimizeMatter || !parent.Spawned || !OptimizationProject.IsFinished || parent.Map.IsSOS2Space() && parent.Map.GetComponent<ShipMapComp>().ShipMapState != ShipMapState.nominal || parent.Map.mapPawns.AllPawns.Where(p => !p.IsPrisoner && !p.InContainerEnclosed && p.HostileTo(Faction.OfPlayer)).Any())
			{
				return;
			}
			age++;
